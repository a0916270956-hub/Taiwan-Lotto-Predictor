...
def get_number_frequencies():
    ...
    try:
        ...
        # 避免資料庫尚未完全建置時引發錯誤
        return pd.DataFrame({'number': range(1, 50), 'count': [0]*49})
...
if page == "🤖 策略選號預測引擎":
    ...
    st.markdown("結合歷史數據庫 (`2007-2026`)，提供多種統計與資料科學策略為您生成下一期的推薦號碼。")
        ...
        if generate_btn:
            for i in range(generate_count):
                ...
                html_balls = "".join([
                    ...
                    f"box-shadow: 2px 2px 5px rgba(0,0,0,0.3);'>{n:02d}</span>" 
                    for n in picks
                ])
...
# ------------------------------------------
    ...
    try:
        ...
        st.warning("⚠️ 尚未讀取到資料，請確認資料庫是否建置成功。")
...
# ------------------------------------------
    ...
    try:
        ...
        st.warning("⚠️ 尚未讀取到加碼資料，請確認資料庫是否建置成功。")
